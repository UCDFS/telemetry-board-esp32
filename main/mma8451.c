...
{
	return i2c_master_read_slave_reg(I2C_NUM_0, MMA8451_I2C_ADDR, reg, pdata, count);
}
...
{
	return i2c_master_write_slave_reg(I2C_NUM_0, MMA8451_I2C_ADDR,  reg, pdata, count);
}
 ...
 */
esp_err_t mma8451_init(mma8451_config_t)
{
	uint8_t val;

    mma8451_read_reg(WHO_AM_I_REG, &val, 1);
    if (val != 0x1A) {
...
esp_err_t mma8451_read_raw(mma8451_raw_data_t *acc) {
    esp_err_t err = mma8451_read_reg(MMA8451_OUT_X_MSB, (uint8_t *)acc, sizeof(mma8451_raw_data_t));
    if (err != ESP_OK) {
        return err;
    }
