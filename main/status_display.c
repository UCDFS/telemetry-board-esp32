...
static ssd1306_handle_t ssd1306_handle;
...
void status_display_init() {
	// Initialize display
	ssd1306_handle = ssd1306_init(i2c_bus_get(), SSD1306_I2C_ADDR_1);
...
static int telemetry_tx_events;
static int telemetry_tx_packets;
static int telemetry_rx_messages;
static int telemetry_rx_packets;
...
void status_display_task() {
	...
	while (true) {
		...
		ssd1306_clear_screen(ssd1306_handle, 0x00);

		switch (display_mode) {
			case DISPLAY_MODE_DEFAULT: {
				ssd1306_draw_string(ssd1306_handle, 0, 0, "Welcome to", 12, true);
				ssd1306_draw_string(ssd1306_handle, 0, 12, "UCDFS Telemetry", 16, true);
				ssd1306_fill_rectangle(ssd1306_handle, 0, 30, 120, 30, true);

				sprintf(buffer, "WiFi: %s", wifi_connected ? wifi_ssid : "Not Connected");
				ssd1306_draw_string(ssd1306_handle, 0, 40, buffer, 12, true);
				if (wifi_connected) {
					sprintf(buffer, "IP: %s", inet_ntoa(ip));
					ssd1306_draw_string(ssd1306_handle, 0, 52, buffer, 12, true);
				}
				break;
			}
			case DISPLAY_MODE_ACCELEROMETER: {
				ssd1306_draw_string(ssd1306_handle, 0, 0, "Accelerometer & Gyro", 12, true);
				ssd1306_fill_rectangle(ssd1306_handle, 0, 12, 127, 12, true);

				sprintf(buffer, "AX: %4.1f", acc_x);
				ssd1306_draw_string(ssd1306_handle, 0, 16, buffer, 12, true);
				sprintf(buffer, "AY: %4.1f", acc_y);
				ssd1306_draw_string(ssd1306_handle, 0, 28, buffer, 12, true);
				sprintf(buffer, "AZ: %4.1f", acc_z);
				ssd1306_draw_string(ssd1306_handle, 0, 40, buffer, 12, true);

				sprintf(buffer, "GX: %4.1f", gyro_x);
				ssd1306_draw_string(ssd1306_handle, 64, 16, buffer, 12, true);
				sprintf(buffer, "GY: %4.1f", gyro_y);
				ssd1306_draw_string(ssd1306_handle, 64, 28, buffer, 12, true);
				sprintf(buffer, "GZ: %4.1f", gyro_z);
				ssd1306_draw_string(ssd1306_handle, 64, 40, buffer, 12, true);
				delay = 20;
				break;
			}
			case DISPLAY_MODE_BRAKE: {
				ssd1306_draw_string(ssd1306_handle, 0, 0, "Brake Temperature", 12, true);
				ssd1306_fill_rectangle(ssd1306_handle, 0, 12, 127, 12, true);
			...
			case DISPLAY_MODE_GPS: {
				ssd1306_draw_string(ssd1306_handle, 0, 0, "GPS", 12, true);
				ssd1306_fill_rectangle(ssd1306_handle, 0, 12, 127, 12, true);
				...
				ssd1306_fill_rectangle(ssd1306_handle, 106, 16, 106, 24, true);
				ssd1306_fill_rectangle(ssd1306_handle, 102, 20, 110, 20, true);
				ssd1306_fill_rectangle(ssd1306_handle, 102, 26, 110, 26, true);
				sprintf(buffer, "%5.2fm", fabsf(gps_acc));
				ssd1306_draw_string(ssd1306_handle, 90, 28, buffer, 12, true);
			...
			case DISPLAY_MODE_TELEMETRY: {
				ssd1306_draw_string(ssd1306_handle, 0, 0, "Telemetry", 12, true);
				ssd1306_fill_rectangle(ssd1306_handle, 0, 12, 127, 12, true);

				sprintf(buffer, "Events: %d", telemetry_tx_events);
				ssd1306_draw_string(ssd1306_handle, 0, 16, buffer, 12, true);
				sprintf(buffer, "TX Packets: %d", telemetry_tx_packets);
				ssd1306_draw_string(ssd1306_handle, 0, 28, buffer, 12, true);

				sprintf(buffer, "Messages: %d", telemetry_rx_messages);
				ssd1306_draw_string(ssd1306_handle, 0, 40, buffer, 12, true);
				sprintf(buffer, "RX Packets: %d", telemetry_rx_packets);
				ssd1306_draw_string(ssd1306_handle, 0, 52, buffer, 12, true);
				delay = 25;
				break;
			}
			case DISPLAY_MODE_TEMPERATURE: {
				ssd1306_draw_string(ssd1306_handle, 0, 0, "Ambient", 12, true);
				ssd1306_fill_rectangle(ssd1306_handle, 0, 12, 127, 12, true);
		...
		ssd1306_refresh(ssd1306_handle);
...
void status_display_event_handler(telemetry_event_type_t event_type, telemetry_event_subtype_t event_subtype, void *data_ptr,
	...
	if (event_type == EVENT_TYPE_SYSTEM) {
		if (event_subtype == EVENT_TYPE_SYSTEM_TELEMETRY_PACKET_SENT) {
			telemetry_tx_packets++;
		}
		if (event_subtype == EVENT_TYPE_SYSTEM_TELEMETRY_PACKET_RECEIVED) {
			telemetry_rx_packets++;
		}

		if (event_subtype == EVENT_TYPE_SYSTEM_TELEMETRY_EVENT_SENT) {
			telemetry_tx_events++;
		}
		if (event_subtype == EVENT_TYPE_SYSTEM_TELEMETRY_MESSAGE_RECEIVED) {
			telemetry_rx_messages++;
		}
	...
	if (event_type == EVENT_TYPE_POSITION) {
		if (event_subtype == EVENT_TYPE_POSITION_GPS_COORDINATES) {
			t_ev_pos_gps_pvt *t_ev = (t_ev_pos_gps_pvt *) data_ptr;
			gps_lat = (float) (t_ev->latitude / 10000000.0);
			gps_lon = (float) (t_ev->longitude / 10000000.0);
			gps_speed = (float) (t_ev->ground_speed / 277.778);
			gps_acc = (float) (t_ev->horizontal_accuracy / 1000.0);
		}
	}
...
esp_err_t status_display_http_handler(httpd_req_t *req) {
	...
	if (ssd1306_handle != NULL) ssd1306_clear_screen(ssd1306_handle, 0x00);
